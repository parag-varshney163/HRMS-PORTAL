...
export default function UpdateBalanceModal({ open, onClose, onSubmit }) {
  ...
  >
    <div
      className="w-full max-w-md rounded-2xl shadow-2xl flex flex-col overflow-visible border"
      style={{
    ...
    >
      {/* Header */}
      <div
        className="flex items-center justify-between p-5 rounded-t-2xl shrink-0 border-b"
        style={{
          borderColor: colors.cardBorder,
      ...
      >
        ...
        <button
          onClick={onClose}
          disabled={isSubmitting}
          className="p-2 rounded-lg transition-colors disabled:opacity-50 cursor-pointer border-none bg-transparent"
          style={{
      ...
      <form onSubmit={handleSubmit} className="p-5 space-y-4">
        ...
        >
          Add positive numbers to increase balance and negative numbers to
          decrease it.
        </p>
          ...
          {loadingUsers ? (
            ...
            <FilterDropDown
              ...
              onSelect={(selectedFormattedName) => {
                const user = users.find(
                  (u) =>
                    `${u.name} (${u.userId})` === selectedFormattedName
                );
        ...
        <div className="pt-4 flex gap-3">
          <button
            ...
            onClick={onClose}
            disabled={isSubmitting}
            className="flex-1 py-2.5 rounded-xl border text-sm font-semibold transition disabled:opacity-50"
            style={{
              backgroundColor: colors.inputBg,
              color: colors.textSecondary,
              borderColor: colors.cardBorder,
